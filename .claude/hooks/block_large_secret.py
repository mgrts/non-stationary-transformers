...
(data/ models/ mlruns/ reports/ notebooks/) and obvious secret/binary files. Only
inspects `git add` and `git commit`; everything else passes through. Exit 2 blocks.
"""
...
from _gitutil import diff_paths, has_short_flag, parse_git_invocations, status_paths  # noqa: E402

SIZE_LIMIT = 10 * 1024 * 1024  # 10 MB, matches check-added-large-files --maxkb=10000
# Artifact dirs are .gitignored (data/, models/, mlruns/, notebooks/) plus reports/.
RISKY_DIR = re.compile(r"^(data|models|mlruns|reports|notebooks)(/|$)")
RISKY_FILE = re.compile(
    r"(^|/)(mlruns\.db|id_rsa|credentials\.json)$"
    r"|\.(pem|key|p12|pfx|pkl|pt|pth|ckpt|h5|joblib|npy|npz|parquet|ipynb)$"
    r"|(^|/)\.env(\.|$)"
)
...
def violations(abs_files):
    ...
    for f in abs_files:
        if not os.path.isfile(f):
            continue
        if os.path.basename(f) in (".gitkeep", ".env.example"):  # legit tracked files
            continue
        ...
        if RISKY_DIR.match(rel):
            bad.append(
                (
                    rel,
                    "inside an artifact dir (data/ models/ mlruns/ reports/ notebooks/) "
                    "- keep it out of git",
                )
            )
            continue
        if RISKY_FILE.search(rel):
            bad.append((rel, "looks like a secret, dataset, notebook, or model/binary artifact"))
            continue
        try:
...
def main() -> int:
    ...
    sys.stderr.write(
        ...
        "  - Keep data/, models/, mlruns/, reports/, notebooks/ in .gitignore "
        "(do not 'git add -f')\n"
